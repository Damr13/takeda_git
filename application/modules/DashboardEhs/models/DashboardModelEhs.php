...
class DashboardModelEhs extends CI_Model{
  ...
  public function __construct(){
    ...
    public function getJudgementVal($start,$finish){
				$bulanA = date("d-m-Y");
        $q = "SELECT
	        ( SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'VERY HIGH' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) very_high,
	        ( SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'HIGH' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) high,
	        ( SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'MODERATE' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) moderate,
	        ( SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'LOW' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) low,
	        ( SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'VERY LOW' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) very_low";
    ...
    public function getStatusByPeople($status,$people){
        ...
        $q = "SELECT DISTINCT * 
							...
							AND Nama != '' 
							AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = ('".$bulanA."')
							GROUP BY Nama
							";
        // echo $q;
        $result = $this->db->query($q)->result();
    ...
    public function get_check($statuss,$peoples){
        ...
        $q = "SELECT
							...
							FROM
								last_cek
							WHERE
								category = '$peoples' AND DATE_FORMAT(last_cek, '%d-%m-%Y' ) = '".$bulanA."'
							";
        // echo $q;
        $result = $this->db->query($q)->result();
        return $result;
    ...
    public function getPieValue($col, $start,$finish){
			$bulanA = date("d-m-Y");
      $q = "SELECT
				(SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'VERY HIGH' and category='$col' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) very_high,
				(SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'HIGH' and category='$col' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) high,
				(SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'MODERATE' and category='$col' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) moderate,
				(SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'LOW' and category='$col' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) low,
				(SELECT IFNULL(count(DISTINCT Nama), judgement) FROM screening WHERE judgement = 'VERY LOW' and category='$col' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) very_low,
    		(SELECT IFNULL(count(DISTINCT Nama), 0) AS total FROM screening WHERE category='$col' AND Nama IS NOT NULL AND Nama != '' AND DATE_FORMAT( tgl, '%d-%m-%Y' ) = '".$bulanA."' ) total";
      $result = $this->db->query($q)->result();
      return $result;
    }
		...
		public function getPieValueUncheck($cols){
			$bulanA = date("d-m-Y");
			$q = "SELECT
			(SELECT IFNULL( count( Nik ), category ) FROM last_cek WHERE category = '$cols' AND DATE_FORMAT( last_cek, '%d-%m-%Y' ) = '".$bulanA."') tot_check,
			(SELECT IFNULL( count( Nik ), category ) FROM last_cek WHERE category = '$cols' AND DATE_FORMAT( last_cek, '%d-%m-%Y' ) != '".$bulanA."') tot_uncheck,
