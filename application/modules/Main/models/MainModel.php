	...
	public function overall_oee_month($tgl,$id_machine){
		...
		$j=0;
			...
			for ($shift=1; $shift <=3 ; $shift++) { 
			    ...
			    if(!empty($lb->id)){
			    	$kolom_prod_det = 'COUNT(*) as jml_d';
				    $tabel_db_prod_det = 'log_book_det_product';
				    $join_prod_det  = '';
				    $where_prod_det = 'where id_log_book = '.$lb->id.' ';
				    $order_prod_det = '';
				    $cek_det_prod = $this->select_row($tabel_db_prod_det,$join_prod_det,$where_prod_det,$kolom_prod_det,$order_prod_det);
				    if($cek_det_prod->jml_d > 0){
				    	$det_prod += 1;
				    }else{
				    	$det_prod += 0;
				    }
		...
		if ($j <= 0){
			...
			$return=array('date' => $tgl,'overall_oee'=>(round(($tot/$det_prod),2)), 'target_oee'=>$target);
		}
