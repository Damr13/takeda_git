<script type="text/javascript">
  ...
  function changePeriodeProd(checkBy) {
    ...
    $.ajax({
      type: "POST",
    	url: '<?php echo base_url().'Main/getTotProdCard/'; ?>',
  ...
  function changeMachine(checkBy) {
    ...
    // LOAD DATA MACHINE AVAILABILITY BASED ON MACHINE --ir 
	  $.ajax({
      type: "POST",
	  	url: '<?php echo base_url().'Main/getMaValue/'; ?>',
      ...
      machineText = $('#id_machine :selected').html();
      $('.oeeSubtitle').html("("+machineText+")")
      $('.availSubtitle').html("("+machineText+")")
      $('.maSubtitle').html("("+machineText+")")
      // $('.availSubtitle').html(machineText)
      // $('.maSubtitle').html(machineText)
  ...
  function atPie2(pot,pdt,udt,it,ut){
    var c3DonutChart = c3.generate({
      bindto: '#c3-donut-chart3',
      data: {
        columns: [
          ['Machine Operating Time', pot],
          ['Plan Downtime', pdt],
          ['Unplan Downtime', udt],
          ['Idle/Waiting Downtime', it],
          ['Utility Downtime', ut],
        ],
        type: 'donut',
  ...
  function calibration(checkBy){
    ...
    if(type){
      $.ajax({
        type: "POST",
        url: '<?php echo base_url().'Main/pm_cal/'; ?>',
  ...
  function capa_qa(checkBy){
    ...
    if(type){
      $.ajax({
        type: "POST",
        url: '<?php echo base_url().'Main/capa/'; ?>',
        data: {type:type, periode:periode, checkBy:checkBy},
        dataType: "json",
        success: function(data) {
          if(data.respone == 'sukses'){
            capa_qa2(data.total,data.extain,data.overdue,data.closed)
          }
        },    
        error: function() {
        }
  ...
  function capa_qa2(total,extain,overdue,closed){
  ...
  function capa_ehs(checkBy){
    ...
    if(type){
      ...
      success: function(data) {
        if(data.respone == 'sukses'){
          capa_ehs2(data.total,data.extain,data.overdue,data.closed)
        }
      },    
      error: function() {
      }
  ...
  function capa_ehs2(total,extain,overdue,closed){
  ...
  function cons_car(checkBy){
    ...
    if(type){
      $.ajax({
      type: "POST",
      url: '<?php echo base_url().'Main/consumption/'; ?>',
      data: {type:type, periode:periode, checkBy:checkBy},
      dataType: "json",
      success: function(data) {
        if(data.respone == 'sukses'){
  ...
  function chart_capex(tahun,compare){
    ...
    $.ajax({
    type: "POST",
    url: '<?php echo base_url().'Main/chart_budget'; ?>',
