...
class Main extends CI_Controller {
	...
	public function Index(){
		...
		$machineLists	= $this->MainModel->machineLists();
...
// 			$tt_prd					+= $p_good + $p_reject;
			...
			$rh							= $this->MainModel->run_hour($bulan);
		...
		$this->load->view('Main/_cjs');
		$this->load->view('Main/_mjs');
		$this->load->view('Main/_data_js');
	}
	...
	public function getTotProdCard(){
		...
		if($checkBy == "periode" && $periode == "year"){
			...
			$tt_prod_good 			= $this->MainModel->product_good_range($week['weekStart'],$week['weekEnd']);
			$p_good							= $tt_prod_good->t_prod;
			$tt_prod_reject 		= $this->MainModel->product_reject_range($week['weekStart'],$week['weekEnd']);
			$p_reject						= $tt_prod_reject->t_prod;
			$tt_prd							= $p_good + $p_reject;
			$rh	 								= $this->MainModel->run_hour_range($week['weekStart'],$week['weekEnd']);
			$run_hour						= $rh->jml_hour;
			...
			$tt_prod_good 			= $this->MainModel->product_good_range($periode[0],$periode[1]);
			$p_good							= $tt_prod_good->t_prod;
			$tt_prod_reject 		= $this->MainModel->product_reject_range($periode[0],$periode[1]);
			$p_reject						= $tt_prod_reject->t_prod;
			$tt_prd							= $p_good + $p_reject;
			$rh	 								= $this->MainModel->run_hour_range($periode[0],$periode[1]);
			$run_hour						= $rh->jml_hour;
		}
		...
		if ($checkOn == "periode") {
			for ($i=$startDate; $i <= $endDate; $i++) {
				...
				for ($shift=1; $shift <= 3 ; $shift++) { 
					$check 	= $this->MainModel->checkLogbook($shift,$date,$checkBy);
					if ($check->count > 0) {
						$countPot = $this->MainModel->countDowntime($checkBy,$shift,$date,'RT',$machine);
						$countPdt = $this->MainModel->countDowntime($checkBy,$shift,$date,'PDT',$machine);
						$countUdt = $this->MainModel->countDowntime($checkBy,$shift,$date,'UDT',$machine);
						$countIt 	= $this->MainModel->countDowntime($checkBy,$shift,$date,'IT',$machine);
						$countUt 	= $this->MainModel->countDowntime($checkBy,$shift,$date,'UT',$machine);
						$pot 			+=  $countPot->total;
						$pdt 			+=  $countPdt->total;
						$udt 			+=  $countUdt->total;
						$it  			+=  $countIt->total;
	...
	public function g_oee(){
	    ...
	    // echo $month;exit();
	    if($periode == 'year'){
	    	// $get   = $this->MainModel->get_oee($id_machine,$tahun);
	    	$get   = $this->MainModel->get_oee2($id_machine,$tahun);
	    }else{
	    	$get   = $this->MainModel->get_oee_month2($id_machine,$month);
	    }
	...
	public function pm_cal(){
	    ...
	    $act = 0;
	    $target = 0;
	    $get_cal   	= $this->MainModel->get_cal($type,$tahun,$periode,$checkBy);
	    if($get_cal){
	...
	public function capa(){
	    ...
	    $overdue = 0;
	    $closed = 0;
	    $get_capa   = $this->MainModel->get_capa($type,$tahun,$periode, $checkBy);
	    if($get_capa){
	    	$total 		= $get_capa->total;
	    	$extain		= $get_capa->extain;
	    	$overdue	= $get_capa->overdue;
	...
	public function consumption(){
	    ...
	    $target = 0;
	    $actual = 0;
	    $get_cons   = $this->MainModel->get_cons($type,$tahun,$periode, $checkBy);
	    if($get_cons){
	...
	public function chart_budget(){
	    ...
	    $act = 0;
	    $target = 0;
	    $get  = $this->MainModel->get_budget($tahun,$compare,$type);
	    if($get){
