...
class Dashboard extends CI_Controller {
	...
	public function Index(){
		...
		$machineLists	= $this->DashboardModel->machineLists();
		...
		for ($i=1; $i <= date('m'); $i++) { 
			...
			$rh							= $this->DashboardModel->run_hour($bulan);
		...
		$this->load->view('Dashboard/_cjs');
		$this->load->view('Dashboard/_mjs');
		$this->load->view('Dashboard/_data_js');
	}
	...
	public function getTotProdCard(){
		...
		if($checkBy == "periode" && $periode == "year"){
			...
			$tt_prod_good 			= $this->DashboardModel->product_good_range($week['weekStart'],$week['weekEnd']);
			$p_good							= $tt_prod_good->t_prod;
			$tt_prod_reject 		= $this->DashboardModel->product_reject_range($week['weekStart'],$week['weekEnd']);
			$p_reject						= $tt_prod_reject->t_prod;
			$tt_prd							= $p_good + $p_reject;
			$rh	 								= $this->DashboardModel->run_hour_range($week['weekStart'],$week['weekEnd']);
			$run_hour						= $rh->jml_hour;
			...
			$tt_prod_good 			= $this->DashboardModel->product_good_range($periode[0],$periode[1]);
			$p_good							= $tt_prod_good->t_prod;
			$tt_prod_reject 		= $this->DashboardModel->product_reject_range($periode[0],$periode[1]);
			$p_reject						= $tt_prod_reject->t_prod;
			$tt_prd							= $p_good + $p_reject;
			$rh	 								= $this->DashboardModel->run_hour_range($periode[0],$periode[1]);
			$run_hour						= $rh->jml_hour;
		}
		...
		if ($checkOn == "periode") {
			for ($i=$startDate; $i <= $endDate; $i++) {
				...
				for ($shift=1; $shift <= 3 ; $shift++) { 
					$check 	= $this->DashboardModel->checkLogbook($shift,$date,$checkBy);
					if ($check->count > 0) {
						$countPot = $this->DashboardModel->countDowntime($checkBy,$shift,$date,'RT',$machine);
						$countPdt = $this->DashboardModel->countDowntime($checkBy,$shift,$date,'PDT',$machine);
						$countUdt = $this->DashboardModel->countDowntime($checkBy,$shift,$date,'UDT',$machine);
						$countIt 	= $this->DashboardModel->countDowntime($checkBy,$shift,$date,'IT',$machine);
						$countUt 	= $this->DashboardModel->countDowntime($checkBy,$shift,$date,'UT',$machine);
						$pot 			+=  $countPot->total;
						$pdt 			+=  $countPdt->total;
						$udt 			+=  $countUdt->total;
						$it  			+=  $countIt->total;
	...
	public function g_oee(){
	    ...
	    // echo $month;exit();
	    if($periode == 'year'){
	    	$get   = $this->DashboardModel->get_oee($id_machine,$tahun);
	    }else{
	    	$get   = $this->DashboardModel->get_oee_month($id_machine,$month);
	    }
	...
	public function pm_cal(){
	    ...
	    $act = 0;
	    $target = 0;
	    $get_cal   	= $this->DashboardModel->get_cal($type,$tahun,$periode,$checkBy);
	    if($get_cal){
	...
	public function capa(){
	    ...
	    $overdue = 0;
	    $closed = 0;
	    $get_capa   = $this->DashboardModel->get_capa($type,$tahun,$periode, $checkBy);
	    if($get_capa){
	    	$total 		= $get_capa->total;
	    	$extain		= $get_capa->extain;
	    	$overdue	= $get_capa->extain;
	    	$closed		= $get_capa->closed;
	    }
	    $respone = "sukses";
		
		$return = array('respone' => $respone, 'total' => $total, 'extain' => $extain, 'overdue' => $overdue, 'closed' => $closed);
		echo json_encode($return);
	}
	...
	public function consumption(){
	    ...
	    $target = 0;
	    $actual = 0;
	    $get_cons   = $this->DashboardModel->get_cons($type,$tahun,$periode, $checkBy);
	    if($get_cons){
	...
	public function chart_budget(){
	    ...
	    $act = 0;
	    $target = 0;
	    $get  = $this->DashboardModel->get_budget($tahun,$compare,$type);
	    if($get){
	    	// $total=$get_cal->target+$get_cal->actual;
